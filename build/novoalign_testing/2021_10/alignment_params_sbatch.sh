...
if [ "$option" == "beds" ]; then \
    for test_id in {1..9}; do \
        ...
        echo "sbatch --job-name=${job_id}.${test_id} --cpus-per-task=32 --verbose --error=$log_dir/test${test_id}/err/${job_id}_test${test_id}.${split_id}.err --output=$log_dir/test${test_id}/err/${job_id}_test${test_id}.${split_id}.out --mem=50g --gres=lscratch:50 --time 01-00:00:00 $sh_job;" >> $sbatch_job
    done
fi