...
parent_dir=""
variations_file=paste0(parent_dir,"STAR_variations.txt")
cigar_dir=paste0(parent_dir,"cigar/")
align_dir=paste0(parent_dir,"align/")

sample_list=c("2h", "2i","2j", "ClipSeqTools_v1","ClipSeqTools_v2","clipv1_double","clipv1_triple","2e","2e_double")
```
...
kable(variations_summary[c(1:18,30:36),c(1,11,14,15,16,19,20)])
kable(variations_summary[c(1:18,30:36),c(1,25:32)])
...
```{r echo=FALSE}
#read in df
align_df=read.csv(paste0(parent_dir,"time.txt"),sep="\t")
colnames(align_df)=c("Started job on","Finished on","Mapping speed, Million of reads per hour","Average input read length","Uniquely mapped reads number" "Uniquely mapped reads %" "Number of splices: Total" "Number of splices: Annotated (sjdb)" 
"Number of reads mapped to multiple loci" "% of reads mapped to multiple loci" "Number of reads mapped to too many loci" "% of reads unmapped: too many mismatches" "% of reads unmapped: too short" "% of reads unmapped: other")
align_df=align_df[,c(-1,-6)]

#subset df for sample_list
align_df=subset(align_df,"version_id" %in% sample_list)
...
tmp_df[nrow(tmp_df)+1,]=c(3,"clipv1_double",0)
tmp_df[nrow(tmp_df)+1,]=c(3,"clipv1_triple",0)
