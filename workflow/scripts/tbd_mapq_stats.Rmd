...
## Purpose:
- Create histograms comparing the original and corrected mapq scores
...
output_dir = paste0(params$output_dir,"/")
...
MAP_Processing <- function(txt_in,cat){
  ...
  if (class(dat) != "try-error") {
    ...
    file_save = paste(output_dir,samplename,".",cat,".png",sep="")
    ggsave(file_save,p)
    
    #Create and save text file summary 
    file_save = paste(output_dir,samplename,".",cat,".txt",sep="")
    fileConn<-file(file_save)
    #line1 = paste("\n* samplename: ",samplename,"\n\n")
    line1 = paste("\n* Number of ",cat," reads: ",sum(dat$count),"\n\n",sep="")
    line2 = paste("\t + Average MAPQ score (std): ", format(round(mean(dat$score),2),nsmall=2), 
                  " (", format(round(sd(dat$score),2),nsmall = 2),")\n",sep="")
    writeLines(paste(line1,line2,sep=""), fileConn)
    close(fileConn)
  }
}

# MAIN CODE
MAP_Processing(counts_old,"original")
MAP_Processing(counts_new,"corrected")
```
...
## MAPQ Score Comparison for `r params$samplename`
#### Original MAPQ Scores
```{r, echo=FALSE, comment='', results="asis"}
process_score(paste0(output_dir,samplename,".original.txt"),
              paste0(output_dir,samplename,".original.png"))
```

#### Corrected MAPQ Scores
```{r, echo=FALSE, comment='', results="asis"}
process_score(paste0(output_dir,samplename,".corrected.txt"),
              paste0(output_dir,samplename,".corrected.png"))
```
...
```{r, echo=FALSE, comment='', results="asis"}
process_img_diff(paste0(output_dir,samplename,".diff.png"))
process_tab_diff(paste0(output_dir,samplename,".diff.txt"))
```
