...
parser$add_argument("-Sanno","--SampleAnnoDir", dest="SampleAnnoDir", required=TRUE, help="path for dir with annotations for each sample (peakannotation_final_table.txt)")
parser$add_argument("-m","--SampleManifiest", dest="SampleManifiest", required=TRUE, help="path to sample_manifest.tsv")
...
pos_DB = args$pos_DB
neg_DB = args$neg_DB
SampleAnnoDir = args$SampleAnnoDir
...
if(testing=="Y"){
  ...
  SampleManifiest=paste0(wd,'/sample_manifest.tsv')
  ...
  SampleAnnoDir=paste0(wd,'/05_annotation/')
...
source(paste0(pipline_dir,"workflow/scripts/09_peak_annotation_functions.R"))
...
files=list.files(paste0(SampleAnnoDir))
# smpl=paste(
