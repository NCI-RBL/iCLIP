...
############### splice junctions
##########################################################################################
Join_Junction=function(FtrCount,FtrCount_fracJCount) {    
      ...
      FtrCount_trimmed$Junc_Peaks=FtrCount_trimmed$ID%in%Junc_peaks
      print(paste0('Junc_Peaks recorded in outputtable: ',
      (length(Junc_peaks)==sum(FtrCount_trimmed$Junc_Peaks))))
      
      out=list(FtrCount_trimmed,PGene_TBL2,Junc_peaks)
...
##########################################################################################
## need to optomize because reading in tables twice when using different Output_option
## Will not take longer with larger datasets
#gencode or Combined. option refers to which reftable to output. Gencode or rpmk+Custom+refseq
Ref_Prep=function(Output_option){
    ...
    if(Output_option=='gencode'){return(gencode_Anno_RNA_comb)}
    else if(Output_option=='Combined'){return(Anno_RNA_comb)}
    else{print('slection error')}
...
#########################################################################################3
IntronExon_prep=function(gencode_path,intron_path,gencode_transc_path){
  #gencode
...
########################################################################################## 
Assign_CLIP_attributes=function(PeaksdataOut){
...
Join_Junction_Combine=function(Peaksdata2_anno,read_depth,FtrCount_out) { 
    Junc_peaks=FtrCount_out$Junc_peaks
...
#### collapse exon
Collapse_Exon=function(Peaksdata2_anno){    
...
##########################################################################################
correct_lnLc=function(Peaksdata2_anno){
