...
params:
  log_dir: "~/../../Volumes/Wolin/mES_fclip_2_YL_020922_v2.0/log/"
---
...
stats_df=data.frame()
for (f in dir(log_dir)){
  if (grepl("202", f, fixed = TRUE)){
    full_path=paste0(log_dir,f)
    files_in_path=dir(full_path)
    stats_df[nrow(stats_df)+1,"run_id"]=f
    if ("snakemake.stats" %in% files_in_path){
      stats_df[nrow(stats_df),"stats"]="Y"
    } else{
      stats_df[nrow(stats_df),"stats"]="N"
    }
  }
}
```
...
for (rowid in rownames(subset(stats_df,stats=="Y"))){
  ...
  #read in stats file
  stats_file <- fromJSON(file = paste0(log_dir,stats_df[rowid,"run_id"],"/snakemake.stats"))
