...
params:
  peak_in: "/Users/homanpj/OneDrive - National Institutes of Health/Loaner/Wolin/CLIP/6-22-21-HaCaT_fCLIP//14_MAnormx/02_analysis/WT_fCLIP_vs_WT-NOPFA/WT_fCLIP_vs_WT-NOPFA_MAnorm_final_table.txt"
  PeakIdnt: "ALL"
  samplename: 'WT_fCLIP'
  background: 'WT-NOPFA'
  interactive: FALSE
...
rmarkdown::render('/Users/homanpj/OneDrive - National Institutes of Health/Loaner/Wolin/CLIP/CLIPpipeline/iCLIP/workflow/scripts/12_MAnormAnnotation.Rmd',
                        encoding=encoding,
                        output_file=paste0(gsub('\\\\ ',' ','/Users/homanpj/OneDrive - National Institutes of Health/Loaner/Wolin/CLIP/6-22-21-HaCaT_fCLIP/14_MAnorm/02_analysis/WT_fCLIP_vs_WT-NOPFA/'),params$samplename,'_vs_',params$background,'_MAnorm_final_report.html'))
```
...
g2=ggplot(p,aes(x=log2(p[,paste0(samplename,'-normalized_Count')]),y=FC,color=-log10(P_value),
                            text=paste0('ID: ',ID,
                                        '\nHostGene: ',`Same Strand: Host_gene_name`,
                                        '\n ',samplename,' normalized Reads: ', p[,paste0(samplename,'-normalized_Count')],
                                        '\n ',background,' normalized Reads: ', p[,paste0(background,'-normalized_Count')],
...
g3=ggplot(p,aes(x=log2(p[,paste0(samplename,'-normalized_Count')]),y=FC,color=`Both Strand: RNA_type_Classification`,
                            text=paste0('ID: ',ID,
...
if (nrow(p)>0) {
fcount=data.frame(u,1:length(u));colnames(fcount)=c('RNA_Type','Counts_All');fcount$Counts_Unique=NA;fcount$Counts_MM=NA;fcount$Peak_count=NA
for (x in 1:length(u)) {
  pam=p[p$PlotColumn%in%u[x],]
  fcount[x,1]=u[x]
  fcount[x,'Counts_Unique']=colSums(pam[,'Counts_Unique',drop=F],na.rm = T)
  fcount[x,'Counts_Multimappers_Scaled']=colSums(pam[,'Counts_Multimappers_Scaled',drop=F],na.rm = T)
...
if (exists('fcount')) {
#plot
gg2 = ggplot(fcount,aes(fill=RNA_Type,x=RNA,y=Counts_Multimappers_Scaled)) +
...
if (nrow(p)>0) {
  ...
  for (x in 1:length(u)) {
    ...
    nccount[x,'Counts_Multimappers_Scaled']=colSums(pam[,'Counts_Multimappers_Scaled',drop=F],na.rm = T)
...
if (exists('nccount')) {
nccount$RNA=''
gg2 = ggplot(nccount,aes(fill=Same_RNA_Subtype,x=RNA,y=Counts_Multimappers_Scaled) ) +
  geom_bar(stat='identity',width=.5)+theme_classic() +
