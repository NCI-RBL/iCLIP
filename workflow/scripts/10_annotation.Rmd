...
params:
  samplename: "Control"
  peak_in: "/Users/homanpj/OneDrive - National Institutes of Health/Loaner/Wolin/CLIP/CLIPpipeline/14_annotation/peaks/Control_Clip_50nt_peakannotation_completePH.txt"
  output_table: "/Users/homanpj/OneDrive - National Institutes of Health/Loaner/Wolin/CLIP/CLIPpipeline/14_annotation/Control_Clip_50nt_peakannotation_finalPH.txt"
  ntmerge: "50nt"
  readdepth: 5
  species: 'hg38'
  Condense: FALSE
...
#if (params$PeakIdnt=='UNIQUE') {peaksout='/uniquereadpeaks/'} ##repeated below
#if (params$PeakIdnt=='all') {peaksout='/allreadpeaks/'} ##repeated below

###phil - why render the file within the file? 
#rmarkdown::render('CLIP_MD16_BWpipe.Rmd',
#                        encoding=encoding,
#                        output_table=paste0(gsub('\\\\ ',' ',params$OutDIR),peaksout, params$samplename,'_CLIP_',params$PeakIdnt,'Peaks',params$ntmerge,'_peakDepth',params$readdepth,params$NameAdd,'_MD16.html'))
```
...
#INbam=params$INbam ###not used
#INbamMM=params$INbamMM ###not used
#INpeaks=params$INpeaks ###not used
#inMAPQ=params$inMAP Q###not used
ntmerge=params$ntmerge
readdepth=as.numeric(params$readdepth)
#NameAdd=params$NameAdd ##removed
...
#not used - snakemake will keep track of output dirs
# if (params$PeakIdnt=='UNIQUE') {
#   peaksout='/uniquereadpeaks/'
#   } else if (params$PeakIdnt=='ALL') {
#     peaksout='/allreadpeaks/'
#   }

#if (dir.exists(file.path(paste0(OutDIR)))==F) {dir.create(paste0(OutDIR))} ##dirs will be created through snakemake
#if (dir.exists(file.path(paste0(OutDIR,"/",peaksout)))==F) {dir.create(paste0(OutDIR,"/",peaksout))} ##dirs will be created through snakemake
#OutDIR=gsub("\\\\ "," ",OutDIR)

#if (PeakIdnt=='UNIQUE') {peaksout='/uniquereadpeaks/'} ###repeated third time
#if (PeakIdnt=='all') {peaksout='/allreadpeaks/'} ###repeated third time
```
...
###phil need an else statement - maybe just print "no reads remaining" as text or something
...
if (nrow(p)>0) {
  ...
  #plot
  gg = ggplot(p,aes(x=RNA_Type,y=Counts_Unique,fill=RNA_Type)) +
    geom_violin()+theme_classic()+geom_jitter(height = 0, width = 0.1,size=0.5,aes(text=ID_name))+
    scale_y_continuous(trans = "log2") +
...
gg=ggplot(p,aes(x=RNA_Type,y=Counts_fracMM,fill=RNA_Type)) +
  geom_violin()+theme_classic()+geom_jitter(height = 0, width = 0.1,size=1)+
  scale_y_continuous(trans = "log2")+
...
if (nrow(p)>0) {
  ...
  #plot
  gg=ggplot(p,aes(x=RNA_Type,y=Counts_fracMM,fill=RNA_Type) ) +
    geom_violin()+theme_classic()+geom_jitter(height = 0, width = 0.05,size=.5,aes(text=ID_name)) +
    scale_y_continuous(trans = "log2") +
...
#write.table(Peaksdata3_anno,
 # file=paste0(OutDIR,peaksout,samplename,"_CLIP_",params$PeakIdnt,"Peaks",ntmerge,'_peakDepth',params$readdepth,params$NameAdd,"_MD16.txt"), sep = "\t", row.names = FALSE, col.names = T, append = F, quote= #FALSE,na = "")
...
###phil - i think this is not for the project?
