...
if(testing=="Y"){
  ...
  anno_dir_sample=paste0(data_dir,"/diffbind/04_annotation/")
  reftable_path =paste0(data_dir,"/diffbind/config/annotation_config.txt")
  anno_dir_project = paste0(data_dir,"/diffbind/04_annotation/01_project/")
  function_script="~/../../Volumes/sevillas2/git/iCLIP/workflow/scripts/09_peak_annotation_functions.R"
  out_dir = paste0(data_dir,"/diffbind/05_demethod/02_analysis/WT_vs_KO/")
...
##########################################################################################
#set id for files
file_id=paste0(samplename,'_vs_',background,'_')

#read manifest file
sample_df=read.table(samplemanifest,sep = '\t',header = T)
sample_df=subset(sample_df, (group==samplename) | (group==background))

#read in pre-prcoss tables
...
##########################################################################################
#set variable used inside function
anno_dir=anno_dir_project
...
##########################################################################################
DB[,paste0('Conc_',samplename)]=rowMeans(DB[,sample_df[sample_df$group%in%samplename,'sample']])
DB[,paste0('Conc_',background)]=rowMeans(DB[,sample_df[sample_df$group%in%background,'sample']])

#clean df
DB=dplyr::rename(DB,
          Length=width)

#merge annotations and DB
...
#clean df
DBdata2_anno=dplyr::rename(DBdata2_anno,
                    FC=Fold,
                    P_value=p.value)
...
if (join_junction) {
  ...
  if (length(samples[samples$group%in%samplename,'sample'])!=length(files)){
    print('Could not find Sample Files')
    print("Join_Junction_Error")
  } else{
    
    # get all spliced read locations  
    #sid= "WT1_fCLIP_peakannotation_final_table.txt" chr1:11080536-11080549_-,chr1:11080763-11080797_-
    splicedreads_Table=as.data.frame(c())
    for (sid in files) {
      anno=fread(paste0(anno_dir_sample,sid), 
                   header=T, sep="\t",stringsAsFactors = F,data.table=F,na.strings = "")
      anno$sample=sid
    ...
    #if there are rows selected in df
    if(nrow(splicedreads_Table[is.na(splicedreads_Table$ID)==F,])>0){
      ...
      splicedreads=separate(as.data.frame(splicedreads),1,sep="_",
                            into=c('chr','strand'),remove = F)%>%
        separate(col='chr',sep=":|-",into=c("chr","start","end"))
      ...
      while (nrow(DBdata_spliced_short)>0) {
        ...
        while( length(setdiff(IDsearch,IDsearchCurrent))>0 & length(setdiff(IDmerge_BDdata,IDmerge_BDdataCurrent))>0){
            ...
            # get individual sample peaks connected to Consensus peak 
        ...
        IDmerge_BDdata = separate(as.data.frame(IDmerge_BDdata),
                                  IDmerge_BDdata,sep = "_",into=c("chr","strand"),remove = F)%>%
        ...
        # Checkpoint
        if(grepl(',',d3$ID)==F){
            print("ERROR_No_conneccted_peaks")
          }
      ...
      # Checkpoint
      if(nrow(DBdata_spliced_combOut[grepl(',',DBdata_spliced_combOut$IDmerge)==F,])>0){
        print("Error_not_Correctly_Joininging_Spliced_Peaks")
      }
...
##########################################################################################
write.table(DBdata2_anno,paste0(out_dir,file_id,'DIFFBIND_post_processing.txt'),sep = "\t",row.names = F)
...
if ('IDmerge'%in% colnames(DBdata2_anno)) {
  DBdata3_anno=DBdata2_anno[,c("ID","IDmerge",'Length','FC','P_value',
                               paste0(c(samplename,background),'_AvgNormCounts'),paste0(sample_df$sample,'_NormCounts'),
                               "Same_gene_name_comb","Same_ensembl_gene_id","Same_type_comb",
                               "Same_Repeat_comb",'Same_feature',"Same_Comb_type_exon","Same_Comb_type_ncRNA",
                               "Oppo_gene_name_comb","Oppo_ensembl_gene_id","Oppo_type_comb",
                               "Oppo_Repeat_comb",'Oppo_feature',"Oppo_Comb_type_exon","Oppo_Comb_type_ncRNA",
                               "Comb_type_exon_Oppo")]
} else {
  DBdata3_anno=DBdata2_anno[,c("ID",'Length','FC','P_value',
                               ...
                               "Same_Repeat_comb",'Same_feature',"Same_Comb_type_exon","Same_Comb_type_ncRNA",
                               "Oppo_gene_name_comb","Oppo_ensembl_gene_id","Oppo_type_comb",
                               "Oppo_Repeat_comb",'Oppo_feature',"Oppo_Comb_type_exon",
