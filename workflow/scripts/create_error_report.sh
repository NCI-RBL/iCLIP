...
error_snakefile="$(cat *_iCLIP.out | grep -v "output_file_error" | grep -v "Error in" | \
...
#do not include error parameters: printableError
error_rules="$(cat *_iCLIP.out | grep -v "output_file_error" | grep -v "printableError" | grep -i "error in" | wc -l)"
...
if [ $error_snakefile == "0" ] && [ $error_rules == "0" ]; then
  ...
  echo -e "-Pipeline has successfully completed\n"
    ...
    echo "*********************************************" >> error_log.txt
    cat *_iCLIP.out | grep -v "output_file_error" | grep -v "Error in" | grep -v "Error executing" | grep -v "error message" | \
