# sample_id: "WT_fCLIP hg38 "
# PeaksUniq: "/Volumes/data/iCLIP/marco/13_counts/WT_fCLIP_50_unique.txt"
# PeaksFracMM: "/Volumes/data/iCLIP/marco/13_counts/WT_fCLIP_50_all.txt"
...
# AnnoFiles: "/Volumes/iCLIP/ref/CLIP_Anno"
# AnnoScript: "/Volumes/sevillas2/git/iCLIP/workflow/scripts/CLIP_Annotation_MD11_BW.R"
# OutDIR2: "/Volumes/data/iCLIP/marco/14_annotation"
...
refGTF=args[1]
peak_type = args[2] #params$PeakIdnt
peak_unique = args[3] #peak_unique
peak_all = args[4] #params$PeaksFracMM
join_junction = args[5]
read_depth = args[6]
DEmethod = args [7]
sample_id = args[8]
nt_merge = str(args[9]) + "nt"
out_dir = args[10]
...
if (peak_type=="unique") {
  fname="UniquePeaks"
} else if (peak_type=="mm") {
  fname="MultiMapPeaks"
} else if (peak_type=="all") {
  fname="allPeaks"
} else{
  quit()
}
...
if (join_junction==TRUE) {
  ...
  ### Junction ID
  junctionID <- function(j_in,j_name,p_name){
    q = j_in
    s = FtrCount.GR
  ...
  Junc_PL1 = junctionID(jcount1.GR,"_Junction1","_Peaks1")
  Junc_PL2 = junctionID(jcount2.GR,"_Junction2","_Peaks2")
  Junc_PL = cbind(Junc_PL1,Junc_PL2)          
  
  ### identify PEAK ID for Site1 and Site2 location of splice junction
  Junc_PLOut = Junc_PL[,c('PrimaryGene_Junction1','JunctionID_Junction1',
  ...
  PGene_TBL2=(matrix(nrow=nrow(PGene_TBL),ncol=ncol(PGene_TBL)+4))
  colnames(PGene_TBL2)=c(colnames(PGene_TBL),'chr','start','end','linkedID')
...
} else if (join_junction==FALSE) {
  FtrCount_splice_junc=FtrCount_splice_junc[FtrCount_splice_junc$Counts_fracMM>=read_depth,]
}
